...
validate_version() {
  ...
  if [[ ! "${version}" =~ ^[0-9]+\.[0-9]+$ ]]; then
    echo "Release version must use MAJOR.MINOR: ${version}" >&2
    exit 1
  fi
}
...
BUNDLE="dist/zemeckis-${VERSION}.zip"
if [[ ! -f "${BUNDLE}" ]]; then
...
TMP_DIR="$(mktemp -d /private/tmp/zemeckis-release.XXXXXX)"
...
curl --fail-with-body \
  ...
  -F "bundle=@${BUNDLE}" \
  -o "${UPLOAD_RESPONSE}" \
  "${BASE_URL}/api/v1/publisher/upload?name=zemeckis-${VERSION}&publishingType=AUTOMATIC"
